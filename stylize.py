...
def stylize(network, initial, initial_noiseblend, content, styles, keep_colors, iterations,
        ...
        with tf.Session() as sess:
            ...
            for i in range(iterations):
                ...
                if (checkpoint_iterations and i % checkpoint_iterations == 0) or last_step:
                    ...
                    if keep_colors and keep_colors == True: #keep colors unchanged
                        content_temp = np.clip(content, 0, 255)
                        output_temp = np.clip(img_out, 0, 255)
                        output_yuv = np.array(Image.fromarray(output_temp.astype(np.uint8)).convert('YCbCr'))
                        content_yuv = np.array(Image.fromarray(content_temp.astype(np.uint8)).convert('YCbCr'))

                        w, h, _ = content_temp.shape
                        output_new = np.empty((w, h, 3), dtype=np.uint8)
                        output_new[..., 0] = output_yuv[..., 0]
                        output_new[..., 1] = content_yuv[..., 1]
                        output_new[..., 2] = content_yuv[..., 2]

                        img_out = np.array(Image.fromarray(output_new, 'YCbCr').convert('RGB'))
                else:
                    img_out = None
